// Copyright GameDevWare, Denis Zykov 2025

#pragma once

#include "GameData/Formulas/EFormulaValueType.h"
#include "UDotNetInt8.generated.h"

/*
 * Internal struct used for reflection in formulas 
 */
UCLASS(Hidden, NotPlaceable, Abstract)
class CHARON_API UDotNetInt8 : public UObject
{
	GENERATED_BODY()

private:
	UPROPERTY()
	int8 __Literal = 0;
	UPROPERTY()
	TArray<int8> __ArrayLiteral;
public:
	inline static EFormulaValueType TypeCode = EFormulaValueType::Int8;
	
	static FNumericProperty* GetLiteralProperty()
	{
		static FNumericProperty* LiteralProperty = nullptr;
		
		if (LiteralProperty)
		{
			return LiteralProperty;
		}
		return LiteralProperty = CastFieldChecked<FNumericProperty>(StaticClass()->FindPropertyByName(GET_MEMBER_NAME_CHECKED(UDotNetInt8, __Literal)));
	}
	static FArrayProperty* GetArrayProperty()
	{
		static FArrayProperty* ArrayProperty = nullptr;
		
		if (ArrayProperty)
		{
			return ArrayProperty;
		}
		return ArrayProperty = CastFieldChecked<FArrayProperty>(StaticClass()->FindPropertyByName(GET_MEMBER_NAME_CHECKED(UDotNetInt8, __ArrayLiteral)));
	}

	//* static properties */

	UPROPERTY()
	int8 MaxValue = std::numeric_limits<int8>::max();
	UPROPERTY()
	int8 MinValue = std::numeric_limits<int8>::min();
	
	/* static methods */
	
	UFUNCTION()
	static int8 Parse(FString S) {
		return static_cast<int8>(TCString<FString::ElementType>::Atoi(*S));
	}
};